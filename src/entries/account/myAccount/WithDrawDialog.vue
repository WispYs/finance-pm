  ...
  <div class="with-draw-dialog">
    <el-dialog
      ...
      <el-form :model="withDrawData">
        <el-form-item label="收款银行账号" :label-width="formLabelWidth">
        ...
        <el-form-item label="提现金额" :label-width="formLabelWidth">
          ...
          <span class="actual-money">可提现金额<b>{{ formatActualMoney(withDrawData.actualMoney) }}</b>元</span>
    ...
    methods: {
      ...
      formatWithDrawMoney() {
        if(this.withDrawData.withDrawMoney) {
          this.withDrawData.withDrawMoney = parseFloat(this.withDrawData.withDrawMoney).toFixed(2);
        }
      },

      // 格式化金额
      formatActualMoney(num) {
